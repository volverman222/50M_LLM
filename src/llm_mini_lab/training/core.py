...
Import the reusable functions from the training package to configure and run training.
"""
...
def load_tokenizer(name="gpt2", model_path=None):
    """Load a supported tokenizer with a common ``encode``/``decode`` API."""
    if name == "gpt2":
        return tiktoken.get_encoding("gpt2")
    if name != "sp16384":
        raise ValueError(f"Tokenizer no soportado: {name}")
    if model_path is None:
        raise ValueError("--tokenizer-model es obligatorio para sp16384")

    try:
        import sentencepiece as spm
    except ImportError as exc:
        raise ImportError(
            "Para usar sp16384 instala sentencepiece: pip install sentencepiece"
        ) from exc

    model_path = Path(model_path)
    if not model_path.is_file():
        raise FileNotFoundError(f"No se encontró el modelo SentencePiece: {model_path}")
    return spm.SentencePieceProcessor(model_file=str(model_path))


def tokenizer_vocab_size(tokenizer):
    """Return vocabulary size for either tiktoken or SentencePiece."""
    return tokenizer.n_vocab if hasattr(tokenizer, "n_vocab") else tokenizer.vocab_size()


def tokenizer_eot_id(tokenizer):
    """Return the document-boundary token supported by a tokenizer."""
    if hasattr(tokenizer, "eot_token"):
        return tokenizer.eot_token
    eos_id = tokenizer.eos_id()
    if eos_id < 0:
        raise ValueError("El tokenizer SentencePiece no define un token EOS")
    return eos_id
...
class LocalTextDataset(Dataset):
    ...
    def __init__(self, file_path, max_length, encoder=None):
        file_path = Path(file_path)
        if not file_path.is_file():
            raise FileNotFoundError(f"No se encontró el archivo: {file_path}")

        text = file_path.read_text(encoding="utf-8")
        tokenizer = encoder or tiktoken.get_encoding("gpt2")
        if hasattr(tokenizer, "eot_token"):
...
class TokenBlockIterableDataset(IterableDataset):
    ...
    def __init__(self, stream, encoder, max_length=1024, add_eot=True,
        ...
        self.EOT = tokenizer_eot_id(encoder)
...
def create_dataloader_fineweb(batch_size=2, max_length=1024, val_mod=100,
    ...
    enc = encoder or tiktoken.get_encoding("gpt2")
